...
// 🔹 Try multiple free proxies until one works
const tryProxies = async (url: string): Promise<Blob> => {
  const proxies = [
  ...
  for (const proxy of proxies) {
    try {
      ...
      if (res.ok) {
        ...
        // Quick sanity check: detect HTML error pages
        const snippet = await blob.slice(0, 200).text();
        if (snippet.startsWith("<!DOCTYPE html") || snippet.startsWith("<html")) {
          console.warn("Proxy returned HTML instead of file:", proxy);
        ...
        console.warn("Proxy failed (bad status):", proxy, res.status, res.statusText);
      }
    } catch (err) {
      console.warn("Proxy error:", proxy, err);
    }
  }

  throw new Error("All proxies failed to fetch the file.");
};
...
const App: React.FC = () => {
  ...
  const handleUrlSubmit = useCallback(async (rawUrl: string) => {
    ...
    setTestUrl(rawUrl.trim()); // Store original, user-facing URL

    try {
      const blob = await tryProxies(urlToFetch);

      // Heuristic check for zip
